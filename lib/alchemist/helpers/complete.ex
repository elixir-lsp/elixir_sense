defmodule Alchemist.Helpers.Complete do
  ...
  def run(exp) do
    ...
    case {status, result, list} do
      ...
      {:yes, [], _} ->
        List.insert_at(list, 0, %{type: :hint, value: "#{exp}"})

      {:yes, _, []} ->
        run(code ++ result)

      {:yes, _, _} ->
        List.insert_at(run(code ++ result), 1, Enum.at(list, 0))
        #
    end
  end
    ...
    for module <- modules, module != Elixir do
      ...
      for {f, a} <- funs,
          {func_kind, fun_args, desc, spec} =
            Map.get(funs_info, {f, a}, {:undefined, "", "", ""}),
          accept_function.(context_module, module, f, a, func_kind) do
        kind =
          case {context_module, module, func_kind} do
            {m, m, :defmacro} ->
              "public_macro"
            ...
            {m, m, :undefined} ->
              if {f, a} in @builtin_functions or exported?.(module, f, a),
                do: "public_function",
            ...
            case :lists.keyfind(f, 1, acc) do
              {f, aa, func_kind, docs, specs} ->
                :lists.keyreplace(
                  ...
                  1,
                  acc,
                  {f, [a | aa], func_kind, [doc | docs], [spec | specs]}
                )
  ...
  defp get_module_funs(mod) do
    if function_exported?(mod, :__info__, 1) do
      funs =
        if docs = Introspection.get_docs(mod, :docs) do
