defmodule ElixirSense.Core.Compiler do
  ...
  def expand(ast, env) do
    expand(ast, %{attrs: [], remotes: [], locals: [], definitions: [], vars: []}, env)
  end
  ...
  def expand({:->, _meta, [_, _]}, _s, _e), do: raise("unhandled_arrow_op")

  def expand({:"::", _meta, [_, _]}, _s, _e), do: raise("unhandled_type_op")

  def expand({:|, _meta, [_, _]}, _s, _e), do: raise("unhandled_cons_op")

  ## __block__

  def expand({:__block__, _meta, []}, s, e), do: {nil, s, e}

  def expand({:__block__, _meta, [arg]}, s, e) do
    # s = s |> add_current_env_to_line(Keyword.fetch!(meta, :line), e)
    expand(arg, s, e)
  end

  def expand({:__block__, meta, args}, s, e) when is_list(args) do
  ...
  def expand({:__MODULE__, meta, ctx}, state, env) when is_atom(ctx) do
  ...
  def expand({:__DIR__, meta, ctx}, state, env) when is_atom(ctx) do
    line = Keyword.get(meta, :line, 0)
    state = if line > 0, do: add_current_env_to_line(state, line, env), else: state
  ...
  def expand({:__CALLER__, meta, ctx} = caller, s, e) when is_atom(ctx) do
  ...
  def expand({:__STACKTRACE__, meta, ctx} = stacktrace, s, e) when is_atom(ctx) do
  ...
  def expand({:__ENV__, meta, ctx}, s, e) when is_atom(ctx) do
  ...
  def expand({{:., dot_meta, [{:__ENV__, meta, atom}, field]}, call_meta, []}, s, e)
  ...
  def expand({unquote_call, _meta, [_]}, _s, _e)
      when unquote_call in [:unquote, :unquote_splicing],
      do: raise("unquote_outside_quote")

  def expand({:quote, meta, [opts]}, s, e) when is_list(opts) do
  ...
  def expand({:quote, _meta, [_]}, _s, _e), do: raise("invalid_args")

  def expand({:quote, meta, [opts, do_block]}, s, e) when is_list(do_block) do
    exprs =
  ...
  def expand({:&, meta, [{:super, super_meta, args} = expr]}, s, e) when is_list(args) do
  ...
  def expand({:&, meta, [{:/, arity_meta, [{:super, super_meta, context}, arity]} = expr]}, s, e)
  ...
  def expand({:&, meta, [arg]}, s, e) do
    assert_no_match_or_guard_scope(e.context, "&")
    expand_fn_capture(meta, arg, s, e)
  end

  def expand({:fn, meta, pairs}, s, e) do
  ...
  def expand({:cond, meta, [opts]}, s, e) do
  ...
  def expand({:try, meta, [opts]}, s, e) do
    assert_no_match_or_guard_scope(e.context, "try")
    {e_clauses, sc, ec} = __MODULE__.Clauses.try(meta, opts, s, e)
    {{:try, meta, [e_clauses]}, sc, ec}
  end

  def expand({:for, _, [_ | _]} = expr, s, e), do: expand_for(expr, s, e, true)

  def expand({:with, meta, [_ | _] = args}, s, e) do
  ...
  def expand({:super, meta, args}, s, e) when is_list(args) do
  ...
  def expand({:^, meta, [arg]}, %{prematch: {prematch, _, _}, vars: {_, write}} = s, e) do
  ...
  def expand({:_, _meta, kind} = var, s, %{context: _context} = e) when is_atom(kind) do
    # if context != :match, do: function_error(meta, e, __MODULE__, :unbound_underscore)
    {var, s, e}
  end

  def expand({:_, _meta, kind} = var, s, %{context: :match} = e) when is_atom(kind) do
    {var, s, e}
  end

  def expand({name, meta, kind}, s, %{context: :match} = e)
  ...
  def expand({name, meta, kind}, s, e) when is_atom(name) and is_atom(kind) do
  ...
  def expand({fun, meta, args}, state, env)
  ...
  def expand({{:., dot_meta, [module, fun]}, meta, args}, state, env)
  ...
  def expand({_, meta, args} = _invalid, _s, _e) when is_list(meta) and is_list(args) do
    raise "invalid_call"
  end
  ...
  def expand(pid, s, e) when is_pid(pid) do
    case e.function do
      nil ->
        {pid, s, e}
  ...
  # def expand(0.0 = zero, s, %{context: :match} = e) when is_float(zero) do
  ...
  defp expand_fn_capture(meta, arg, s, e) do
    case __MODULE__.Fn.capture(meta, arg, s, e) |> dbg do
    ...
    {{e_expr, se, ee}, normalized_opts} =
      case validate_for_options(e_opts, false, false, false, return, meta, e, []) do
        {:ok, maybe_reduce, nopts} ->
          {expand_for_do_block(meta, expr, sc, ec, maybe_reduce), nopts}
  ...
  defp expand_for_do_block(Meta, [{:->, _, _} | _], _S, E, false),
  ...
  defp expand_for_generator(x, s, e), do: expand(x, s, e)
  ...
  defmodule Fn do
    ...
    defp escape({:&, _, [pos]}, _e, dict) when is_integer(pos) and pos > 0 do
