defmodule ElixirSense.Core.ExCkReader do
  ...
  Signatures are cached per-module with a configurable TTL: successes default
  to 5 minutes (`:exck_cache_ttl`), while negative results default to 1 second
  (`:exck_negative_cache_ttl`) so that a module queried just before it is
  (re)compiled doesn't keep serving a stale "no chunk" answer for minutes.
  ...
  defp fresh?(result, inserted) do
    diff = current_time() - inserted
    diff <= ttl_for(result)
  end

  # Negative results ({:error, :beam_not_found} / :not_found / decode failures)
  # get a much shorter TTL than successes: in an editor session a project
  # module is often queried moments before it is (re)compiled, and a
  # full-TTL negative entry would suppress ExCk-backed types for minutes
  # after the BEAM appears. Successes stay long-lived — a stale positive is
  # at worst slightly outdated signatures, while a stale negative hides the
  # feature entirely.
  defp ttl_for({:error, _}), do: negative_ttl_ms()
  defp ttl_for(_result), do: ttl_ms()
  ...
  defp negative_ttl_ms do
    case Application.get_env(:elixir_sense, :exck_negative_cache_ttl) do
      ttl when is_integer(ttl) and ttl >= 0 -> ttl
      _ -> 1_000
    end
  end
