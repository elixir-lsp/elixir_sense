...
defmodule ElixirSense.Providers.Completion.CompletionEngine do
  @moduledoc """
  ...
  def get_module_funs(mod, include_builtin) do
    ...
    if docs != nil and function_exported?(mod, :__info__, 1) do
      ...
      for {f, a} <- exports do
        ...
        # Several Kernel.SpecialForms macros (e.g. `%{}`, `.`, `__aliases__`,
        # `__block__`, `fn`, `unquote_splicing`) ship doc signatures that
        # either don't parse as a function call or parse to a different arity
        # than the macro's actual one. When the recovered arg list doesn't
        # match the arity, synthesize a dummy one.
        fun_args =
  ...
  defp match_map_fields(fields, hint, type, %State.Env{} = _env, %Metadata{} = metadata) do
    {subtype, origin, types} =
      case type do
        {:struct, {:atom, mod}} ->
          types =
            ElixirSense.Providers.Utils.Field.get_field_types(
              metadata,
              mod,
              true
            )
