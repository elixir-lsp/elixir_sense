defmodule CompleteTest do
  ...
  test "return completion candidates for 'List'" do
    assert run('List') == [
      'List.;hint',
      'List;module;;Specialized functions that only work on lists.',
      ...
      'to_atom/1;function;charlist;List;Converts a charlist to an atom.;@spec to_atom(charlist) :: atom',
      'to_existing_atom/1;function;charlist;List;Converts a charlist to an existing atom. Raises an `ArgumentError`\\nif the atom does not exist.;@spec to_existing_atom(charlist) :: atom',
      'to_float/1;function;charlist;List;Returns the float whose text representation is `charlist`.;@spec to_float(charlist) :: float',
      ...
      'zip/1;function;list_of_lists;List;Zips corresponding elements from each list in `list_of_lists`.;@spec zip([list]) :: [tuple]',
      'module_info/1;function;;List;;',
      'module_info/0;function;;List;;',
      ...
      'to_integer/2;function;charlist,base;List;Returns an integer whose text representation is `charlist` in base `base`.;@spec to_integer(charlist, 2..36) :: integer',
      'to_integer/1;function;charlist;List;Returns an integer whose text representation is `charlist`.;@spec to_integer(charlist) :: integer',
