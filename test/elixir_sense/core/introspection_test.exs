  ...
  test "get_returns_from_callback" do
    ...
    assert returns == [
             %{
               ...
               spec: "{:ok, new_state :: term} when old_vsn: term"
             },
             %{
               description: "{:error, reason}",
               snippet: "{:error, \"${1:reason}$\"}",
               spec: "{:error, reason :: term} when old_vsn: term"
             },
             %{
               description: "{:down, term}",
               snippet: "{:down, term()}",
               spec: "{:down, term} when old_vsn: term"
             }
           ]
  end
